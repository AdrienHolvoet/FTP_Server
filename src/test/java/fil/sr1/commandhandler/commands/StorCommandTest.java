...
public class StorCommandTest extends CommandHandlerTest {
    StorCommand command;
    ...
    public void setupBeforeEach() {
        ...
        command = new StorCommand(TMP_FILE_ONE, clientFtp);
    ...
    public void sendMessage125And226Ok_whenRetrIsSuccessful() throws IOException {
        Mockito.doNothing().when(dataSocket).storeFile(Mockito.<File>any());
        command.handle();
    ...
    public void sendMessage451Nok_whenIOExceptionIsThrownWhileTryingToRetrieveFile() throws IOException {
        Mockito.doThrow(new IOException()).when(dataSocket).storeFile(Mockito.<File>any());
        command.handle();
